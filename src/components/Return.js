...
export default function Return() {
  ...
  const getBooks = () => {
    fetch("http://127.0.0.1:8000/api/returnNotConfirm")
      .then((response) => {
  ...
  const acceptRequest = (id) => {
    confirm({
      title: "Are you sure wanna to accept this return request?",
      icon: <ExclamationCircleOutlined />,
      okText: "Yes",
      okType: "danger",
      cancelText: "Cancel",
      onOk() {
        fetch("http://127.0.0.1:8000/api/acceptReturn/" + id, {
          method: "put",
          headers: {
  ...
  const declineRequest = (id, book_id) => {
    confirm({
      title: "Are you sure wanna to decline this return request?",
      icon: <ExclamationCircleOutlined />,
      okText: "Yes",
      okType: "danger",
      cancelText: "Cancel",
      onOk() {
        fetch("http://127.0.0.1:8000/api/declineReturn/" + id, {
          method: "put",
          headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
